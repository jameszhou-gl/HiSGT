...
parser = argparse.ArgumentParser(
    description='Sample sequences from the trained HALOCoarse model')
...
# for k, l in enumerate(id_to_label):
  index_to_code[args.code_vocab_size+int(idx)] = f"Chronic Condition: {label}"

model = HALOCoarseModel(args).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)

checkpoint = torch.load(f'{args.LOG_DIR}/{args.dataset}_{args.dataset_version}_haloCoarse.pt', map_location=device)
...
def convert_ehr(ehrs, index_to_code=None):
  ...
  for i in range(len(ehrs)):
    ehr = ehrs[i]
    ehr_output = []
    labels_output = ehr[1][args.code_vocab_size:args.code_vocab_size+args.label_vocab_size]
    if index_to_code is not None:
      labels_output = [index_to_code[idx + args.code_vocab_size] for idx in np.nonzero(labels_output)[0]]
...
# synthetic_ehrs = sample_sequence(model, args.n_ctx, stoken, batch_size=3, device=device, sample=True)
...
for i in tqdm(range(0, len(train_ehr_dataset), args.batch_size)):
  ...
  batch_synthetic_ehrs = sample_sequence(model, args.n_ctx, stoken, batch_size=bs, device=device, sample=True)
...
pickle.dump(synthetic_ehr_dataset, open(f'{args.LOG_DIR}/{args.dataset}_{args.dataset_version}_haloCoarse.pkl', 'wb'))
