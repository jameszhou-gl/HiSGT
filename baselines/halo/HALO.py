...
class CoarseTransformerModel(nn.Module):
    ...
    def forward(self, input_visits, position_ids=None, past=None):
        ...
        hidden_states = inputs_embeds + position_embeds  # [48, 48, 768]
...
class AutoregressiveLinear(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=True):
        ...
        # ! note: actually, mask here is indeed a upper triangular matrix not lower one, but in F.linear, self.mask*self.weight would be transposed, equalling to upper triangular matrix.
        self.register_buffer('mask', torch.tril(
            torch.ones(in_features, out_features)).int())
...
class FineAutoregressiveHead(nn.Module):
    def __init__(self, config):
        ...
        self.auto1 = AutoregressiveLinear(
            config.n_embd + config.total_vocab_size, config.n_embd + config.total_vocab_size)
    ...
    def forward(self, history, input_visits):
        ...
        code_logits = self.auto2(torch.relu(self.auto1(
            torch.cat((history, input_visits), dim=2))))[:, :, self.n_embd-1:-1]
        return code_logits

    def sample(self, history, input_visits):
        history = history[:, :-1, :]
        input_visits = input_visits[:, 1:, :]
        # ? why only based on current visit, rather than torch.cat((history, input_visits) to auto1
        # It will be the same in matrix operation between passing currVisit into auto1 and auto2, with passing all visits like forward() and then choosing the last visit.
        currVisit = torch.cat((history, input_visits), dim=2)[
            :, -1, :].unsqueeze(1)
...
class HALOModel(nn.Module):
        ...
        if ehr_labels is not None:  # torch.Size([48, 48, 7012])
            ...
            if pos_loss_weight is not None:
                ...
                loss_weights = loss_weights + \
                    (pos_loss_weight-1) * shift_labels
            if ehr_masks is not None:  # torch.Size([48, 47, 1])
    ...
    def sample(self, input_visits, random=True):
        ...
        i = 0
        # ! note: while-loop makes sure a autoregressive sequence generation
        # If call self.ehr_head.sample(hidden_states, input_visits) only once to generate the entire visit,
        # it would imply that all the codes in the current visit are generated simultaneously and independently of each other.
        while i < self.ehr_head.tot_vocab:
            ...
            input_visits[:, -1, i + first_nonzero] = visit[:,
                                                           0, i + first_nonzero]
            i = i + first_nonzero + 1
